  ...
  return (
    <div className='h-screen flex flex-col items-center pt-36'>
      <div className='mb-8 px-12'>
        ...
        <p className='text-xl leading-5 text-mainTextColor'><span className='mb-1 block'>Let's Sign you In,</span>You have been really missed.</p>
      </div>
      <form onSubmit={ListenForFormSubmit} className='Auth-form mt-8 flex flex-col justify-end items-end'>
        <div className=' w-fit flex flex-col'>
          <label htmlFor="username-email"><span className='mb-1 ml-3 opacity-30 block'>Username/Email</span><input onChange={ListenForInputChanges} value={HandleInput.email_username} name="email_username" id="username-email" className='h-16 normal:h-20 mb-4 w-64 normal:w-72 semiNormal:w-80 md:w-96 rounded-xl bg-secondaryBackgroundDarkColor  text-lg pl-6 pr-3 outline-none' placeholder='@jhonDoe - jhonDoe@gmail.com' type="text" /></label>
          <label htmlFor="password"><span className='mb-1 ml-3 opacity-30 block'>Password</span><input id="password" onChange={ListenForInputChanges} value={HandleInput.password} name="password" className='h-16 normal:h-20 mb-2 w-64 normal:w-72 semiNormal:w-80 md:w-96 rounded-xl bg-secondaryBackgroundDarkColor  text-lg pl-6 outline-none' type="password" placeholder='Password - eg. ljfw3094wojf' /></label>
        </div>
